    ...
    <table class="data-table">
      ...
      <tbody>
        <tr v-if="destinations.length === 0">
          <td colspan="4" class="no-data">Belum ada destinasi.</td>
        </tr>
        <tr v-for="item in destinations" :key="item.id">
          <td>{{ item.name }}</td>
          <td>{{ item.description ? item.description.substring(0, 100) + '...' : '-' }}</td>
          <td>
            <img v-if="item.imageUrl" :src="item.imageUrl" alt="Destinasi" class="table-image">
            <span v-else>-</span>
          </td>
          <td>
